...
function searchArray(e) {
  let strFinal = ''
  let notFound = true
  let keyword = document.querySelector("#txt-search").value

  if (e.target.id == "btn-show-all") {
    showAll = true
  }
  else {
    showAll = false
  ...
  for (let i = 0; i < numElements; i++) {
    let str = videosList[i].description
    let found = str.includes(keyword)

    if (keyword != "" || showAll) {
      if (found || showAll) {
        strFinal += generateRowHTML(videosList[i])
        notFound = false
      }
    }
    else {
  ...
  if (notFound) {
    let string = "No videos found with keyword "
    string += document.querySelector("#txt-search").value
    document.querySelector("#notFound").className = "open"
    document.querySelector("#notFound").innerHTML = string;
  }
  if (!notFound && keyword != "") {
    document.querySelector("#notFound").className = "hidden"
  }

  document.querySelector(".flexbox").innerHTML = strFinal
}



document.querySelector("#btn-search").addEventListener("click", searchArray)
document.querySelector("#btn-show-all").addEventListener("click", searchArray)
